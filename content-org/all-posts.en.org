...
create tables[fn:orgmode-table-manual] as you would in normal org mode.
...
The source block could be latex or plantuml[fn:plantuml], etc.

#+html: {{< tabs tikz plantuml >}}
  #+html: {{< tab >}}
...
#+ATTR_HTML: :width 30%
#+RESULTS:
[[file:../static/images/posts/Writing-Guide-Org/tikz_example.svg]]
...
you can use cdlatex[fn:cdlatex] to simplify your math typing.
...
#+caption: create new virtual machine with virt-manager
[[file:../static/images/posts/linux-hypervisor-setup/create-new-virtual-machine.png]]
...
[[https://github.com/sky-bro/.dotfiles/blob/master/bin/rofi-vmrun.sh][rofi-vmrun.sh]] is a script that I use to quickly manage my virtual machines.

- alt+o :: open selected vm in virt-manager
- alt+r :: run/stop (toggle) selected vm
- enter :: run selected vm and open it in virt-manager
...
**** mount at startup
   ...
   #+begin_src shell
     ...
     sudo vim /etc/fstab
     # add smbshare like:
     # //192.168.31.248/Dev /home/sky/share/Dev cifs credentials=/home/sky/.smbcredentials,user,exec,uid=sky,gid=sky 0 0
...
* DONE Shortest Paths Algorithms :shortest_path:@Data_Structure_and_Algorithm:
CLOSED: [2024-02-10 Sat 18:04]
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: shortest-paths-algorithms
:EXPORT_FILE_NAME: index
:EXPORT_DATE: [2023-12-15 Fri 10:18]
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :image "/images/icons/bellman-ford-example.png"
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :libraries '(mathjax)
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :description "compare shortest path algorithms: dijkstra, floyd, bellman-ford"
:END:
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-02-10 Sat 18:04]
:END:
...
code example: from [[https://github.com/sky-bro/AC/tree/master/Algorithms/Dijkstra][AC/Algorithms/Dijkstra]]
...
- [[https://www.luogu.com.cn/problem/P2136][P2136 拉近距离]]
- [[https://leetcode.com/problems/cheapest-flights-within-k-stops/][787. Cheapest Flights Within K Stops]]: variant of SPFA, path at most of length K
...
* DONE Key Management With GnuPG             :GnuPG:@Crypto:@workspace_setup:
CLOSED: [2024-06-01 Sat 17:05]
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: key-management-with-gnupg
:EXPORT_FILE_NAME: index
:EXPORT_DATE: [2024-05-31 Fri 21:45]
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :image "/images/icons/GnuPG-Logo.svg"
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :libraries '(mathjax)
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :description "learn how to manage you keys with GPG, and use it with ssh and git and pass."
:END:
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-06-01 Sat 17:05]
:END:

** Introduction

GnuPG [fn:GnuPG] is a tool for key management, and encrypting/decrypting/signing with those keys.
...
- (E)ncrypt: encrypt data, used in [[/posts/manage-passwords-with-pass/][password encryption]]
...
#+html: {{< tabs "Primary Key" "Sub Signing Key" "Sub Authentication Key" "Sub Encryption Key" >}}
  #+html: {{< tab >}}
...
from: [[https://superuser.com/a/624488][Keep GnuPG credentials cached for entire user session]]
...
#+caption: work with ssh example
[[file:../static/images/posts/key-management-with-gnupg/work-with-ssh-example.png]]
...
Please refer to [[/posts/manage-passwords-with-pass/][Manage Passwords with Pass]].

* DONE Forward & Backward Propagation                  :gradient_descent:@AI:
CLOSED: [2026-02-16 Mon 23:26]
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: forward-and-backward-propagation
:EXPORT_FILE_NAME: index
:EXPORT_DATE: [2026-02-15 Sun 09:05]
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :image "/images/icons/gradient-descent.png"
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :libraries '(mathjax)
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :description "understand how backward propagation works in gradient descent."
:END:
:LOGBOOK:
- State "DONE"       from "TODO"       [2026-02-16 Mon 23:26]
:END:

** Deep Neural Network

#+begin_src latex :file ../static/images/posts/forward-backward-propagation/neural_network_example.svg :exports results :results file graphics
  \begin{tikzpicture}[
    >=Stealth,
    neuron/.style={circle, draw, thick, minimum size=1.1cm,
      inner sep=0pt},
    neuron split/.style={neuron, path picture={
          % Draws a vertical line from the top to the bottom of the node
          \draw (path picture bounding box.north) -- (path picture bounding box.south);
      }},
    bias/.style={circle, draw, thick,
      minimum size=1.1cm, inner sep=0pt},
    label style/.style={font=\scriptsize, pos=0.2, fill=none, inner sep=0pt} ]

    % --- LAYER 1: INPUT (3 Neurons + 1 Bias at bottom) ---
    \foreach \i in {1,2,3} { \node[neuron] (L1-\i) at (0, -\i * 2 - 1)
      {$x_\i$}; } \node[bias] (L1-b) at (0, -9) {+1};

    % --- LAYER 2: HIDDEN (3 Neurons + 1 Bias at bottom) ---
    \foreach \j in {1,2,3,4} {
      \node[neuron split] (L2-\j) at (5, -\j * 2) {};
      \node[anchor=east, xshift=0.1cm, font=\scriptsize] at (L2-\j.center) {$z^{(2)}_\j$};
      \node[anchor=west, xshift=-0.1cm, font=\scriptsize] at (L2-\j.center) {$a^{(2)}_\j$};
    }
    \node[bias] (L2-b) at (5, -10) {+1};

    % % --- LAYER 3: OUTPUT (2 Neurons) ---
    \foreach \k in {1,2} {
      \node[neuron split] (L3-\k) at (10, -\k * 2 -
      3) {};
      \node[anchor=east, xshift=0.1cm, font=\scriptsize] at (L3-\k.center) {$z^{(3)}_\k$};
      \node[anchor=west, xshift=-0.1cm, font=\scriptsize] at (L3-\k.center) {$a^{(3)}_\k$};
    }

    % % --- CONNECTIONS: INPUT TO HIDDEN ---
    \foreach \i in {1,2,3} { \foreach \j [evaluate=\j as \mypos using
        {0.17 - 0.11*mod(\j,2)}] in {1,2,3,4} { \draw[->] (L1-\i) --
        (L2-\j) node[label style, pos=\mypos] {$w_{\j,\i}^{(2)}$}; } }
    % % Bias connections to Hidden
    \foreach \j in {1,2,3,4} { \draw[->, dashed] (L1-b) -- (L2-\j)
      node[label style, pos=0.15] {$b_{\j}^{(2)}$}; }

    % % --- CONNECTIONS: HIDDEN TO OUTPUT ---
    \foreach \j in {1,2,3,4} { \foreach \k in {1,2} { \draw[->] (L2-\j)
        -- (L3-\k) node[label style] {$w_{\k,\j}^{(3)}$}; } }
    % % Bias connections to Output
    \foreach \k in {1,2} { \draw[->, dashed] (L2-b) -- (L3-\k) node[label
        style] {$b^{(3)}_\k$}; }
...
#+ATTR_HTML: :width 70%
#+RESULTS:
[[file:../static/images/posts/forward-backward-propagation/neural_network_example.svg]]
...
* TODO minimum spanning tree :@Data_Structure_and_Algorithm:minimum_spanning_tree:
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: minimum-spanning-tree
:EXPORT_FILE_NAME: index
:EXPORT_DATE: [2024-02-11 Sun 16:49]
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :image "/images/icons/tortoise.png"
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :libraries '(mathjax)
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :description "this is a description"
:END:
...
* TODO DSU on tree (Sack)            :Sack:DSU:@Data_Structure_and_Algorithm:
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: dsu-on-tree
:EXPORT_FILE_NAME: index
:EXPORT_DATE: [2024-02-16 Fri 19:05]
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :image "/images/icons/tortoise.png"
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :libraries '(mathjax)
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :description "this is a description"
:END:
...
this algorithm is used for answering queries like: how many vertices in the subtree of vertex v has some property in $O(n\log {n})$

* TODO Claude Code QuickStart                  :claude_code:@workspace_setup:
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: claude-code-quickstart
:EXPORT_FILE_NAME: index
:EXPORT_DATE: [2026-02-14 Sat 18:15]
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :image "/images/icons/tortoise.png"
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :libraries '(mathjax)
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :description "this is a description"
:END:
...
#+html: {{< tabs "~/.claude/settings.json" "~/.config/claude-code-sugar/config.json" >}}
  #+html: {{< tab >}}
...
use hooks[fn:claude-code-hooks] to send notifications when task finished or notification arise. Merge code below to your =~/.claude/settings.json=
...
* DONE Mitmproxy + Tampermonkey = better {llm, ...} viewer :mitmproxy:llm:@proxy:@workspace_setup:
CLOSED: [2026-03-22 Sun 21:49]
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: mitmproxy-plus-tampermonkey-better-llm-dot-dot-dot-viewer
:EXPORT_FILE_NAME: index
:EXPORT_DATE: [2026-03-12 Thu 23:32]
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :image "/images/icons/better-llm-viewer.png"
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :libraries '(mathjax)
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :description "this is a description"
:END:
:LOGBOOK:
- State "DONE"       from "TODO"       [2026-03-22 Sun 21:49]
:END:
...
#+CAPTION: LLM Better View Example
[[file:../static/images/posts/mitmproxy-plus-tampermonkey-better-llm-dot-dot-dot-viewer/llm-better-view-example.png]]

** Capture llm api calls
...
** Install tampermonkey better-llm-view script
:PROPERTIES:
:ID:       6aa540d6-4fd9-450f-97a4-17d51234a977
:END:

I already have a [[https://github.com/sky-bro/mitmproxy-llm-better-view?tab=readme-ov-file#method-2-tampermonkey-script][tampermonkey script]], you can install it directly or develop upon it.
...
#+CAPTION: set script include rule
[[file:../static/images/posts/mitmproxy-plus-tampermonkey-better-llm-dot-dot-dot-viewer/llm-better-view-set-include-rule.png]]

#+caption: allow user scripts for tampermonkey
[[file:../static/images/posts/mitmproxy-plus-tampermonkey-better-llm-dot-dot-dot-viewer/allow-user-script-for-tampermonkey.png]]

** (advanced) create an addon to clean up history captures

Clear capture history with mitm [[https://docs.mitmproxy.org/stable/addons/overview/][addon]]
...
[fn:cdlatex] use [[https://orgmode.org/manual/CDLaTeX-mode.html][CDLaTex mode]] to simplify math typing in latex

[fn:claude-code-hooks] use [[https://code.claude.com/docs/en/hooks][claude code hooks]] to trigger custom scripts

[fn:GnuPG] GNU Privacy Guard is a complete and free implementation of the OpenPGP standard as defined by [[https://www.ietf.org/rfc/rfc4880.txt][RFC4880]].
...
[fn:orgmode-table-manual] [[https://orgmode.org/manual/Tables.html][org mode manual on tables]]
