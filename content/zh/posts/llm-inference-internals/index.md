...
4. [Prefill vs Decode：为什么同一个模型有两个完全不同的瓶颈]({{< relref "/posts/prefill-vs-decode" >}})
5. [Paged Attention：GPU 上的虚拟内存]({{< relref "/posts/paged-attention" >}})
6. [Continuous Batching：按迭代粒度调度]({{< relref "/posts/continuous-batching" >}})
7. [Chunked Prefill：把 Prefill 切片，保护 Decode 延迟]({{< relref "/posts/chunked-prefill" >}})
8. [Prefix Caching：跨请求复用 KV Cache]({{< relref "/posts/prefix-caching" >}})
9. [Disaggregated Prefill：把计算拆到不同机器上]({{< relref "/posts/disaggregated-prefill" >}})
